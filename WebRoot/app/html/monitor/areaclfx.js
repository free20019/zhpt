...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
	...
	//加载查询条件按钮
	dc.place(dc.create("span",{innerHTML:"区域",style:{"margin-right":"5px"}}),'areaclfxdiv');
	var areaclfx_area= new FilteringSelect({id:"areaclfx_area",options: [],style:{"width":"184px","margin-right":"15px"}}).placeAt('areaclfxdiv');
	dc.place(dc.create("span",{innerHTML:"日期",style:{"margin-right":"5px"}}),'areaclfxdiv');
	var areaclfx_time= new TextBox({id:"areaclfx_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('areaclfxdiv');
	var queryCondition = {"id":areaclfx_area,"time":areaclfx_time};
	var areaclfxbutton=new Button({
	        label: "查询",
	        onClick: function(){
				this.showWait();
	...
	new Button({
		...
		onClick : function() {
			...
			url = "hygl/zdjkqyexcle?postData="
				+ dojo.toJson(postData), window.open(url)
		}
	}).placeAt('areaclfxdiv');
	queryArea().then(function(data){
//		console.dir("#####"+data.datas)
		areaclfx_area.set('store',new dojo.store.Memory({ data: data.datas}));
	})
		areaclfx_area.set('queryExpr', "*${0}*"  )
	//添加表格
	var columns = [
		       		{field:'message', 
		    			label : "时间" 
	...
	var xhrArgs = {
			...
			var min=[];
				...
				areaclfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
				store = createSyncStore({
					data : data.DATA
				});
				areaclfxGrid.set('collection', store);
			},
		};
...
function areaclfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){		
	var myChart = echarts.init(document.getElementById('areaclfxtb'));
    // 使用
	...
	echarts.init(document.getElementById('areaclfxtb')).setOption(option);				
}	
function tjnr(list){
	var zhsz=[];
	for(var i=0;i<48;i++){
