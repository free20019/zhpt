...
require([ "app/util","dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select"
	...
	dc.place(dc.create("span",{innerHTML:"车号",style:{"margin-right":"5px"}}),'szdcdiv');
	var szdc_vhic= new TextBox({id:"szdc_vhic",style:{"width":"8em","margin-right":"15px"}}).placeAt('szdcdiv');
	var queryCondition = {"vhic":szdc_vhic,"type":"神州"};
	var szdcbutton=new Button({
	        label: "查询",
	        onClick: function(){
				this.showWait();
...
//	}).placeAt('szdcdiv');
	var columns = [
		...
		var xhrArgs = {
			...
			postData : 'postData={"page":1,"pageSize":30,"type":"神州"}',
			handleAs : "json",
			load : function(data) {
				szdcbutton.hideWait();
				console.log(data.count)
					szdcGrid.totalCount = data.count;
					...
					szdcGrid.set('collection',store);
					szdcGrid.pagination.refresh(data.datas.length);
			},
		};
		var CustomGrid = declare([Grid, Keyboard ,ColumnResizer,Selection]);
		szdcGrid= new CustomGrid({
			totalCount : 0,
			columns: columns,
			pagination:null
		},"szdctable");
		szdcGrid.refresh2 = function(){
			dojo.xhrPost(xhrArgs);
		}
//			dojo.xhrPost(xhrArgs);
		
		var pageii = new Pagination(szdcGrid,xhrArgs,queryCondition,30);
		
		dc.place(pageii.first(),'szdctable','after');
});
