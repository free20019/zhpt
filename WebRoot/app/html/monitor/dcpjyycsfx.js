...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
    ...
    //加载查询条件按钮
    var dcpjyycsfx_time= new TextBox({id:"dcpjyycsfx_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('dcpjyycsfxdiv');
    var queryCondition = {"time":dcpjyycsfx_time};
    var dcpjyycsfxbutton=new Button({
        label: "查询",
        onClick: function(){
            this.showWait();
            ...
            for(var o in queryCondition){
                if(o == "time"){
                    postData[o] = dojo.byId("dcpjyycsfx_time").value;
                }else{
    ...
    new Button({
        ...
        onClick : function() {
            ...
            url = "hygl/finddcyyexcle?postData="
    ...
    //查询按钮
    var xhrArgs = {
        url : basePath + "hygl/finddcyy",
        postData : 'postData={"page":1,"pageSize":'+pageSize+'}',
        handleAs : "json",
        load : function(data) {
            dcpjyycsfxbutton.hideWait();
            var jint = [];
            var zuot = [];
            var qiant=[];
            var shangz=[];
            ...
            dcpjyycsfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
            store = createSyncStore({
                data : data.DATA
            });
            dcpjyycsfxGrid.set('collection', store);
        },
    };
...
function dcpjyycsfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){
    var myChart = echarts.init(document.getElementById('dcpjyycsfxtb'));
    // 使用
    option = {
        title : {
            text: '',
    ...
    echarts.init(document.getElementById('dcpjyycsfxtb')).setOption(option);
}
function tjnr(list){
    var zhsz=[];
    for(var i=0;i<24;i++){
