...
require([ "app/util","dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select"
	...
	dc.place(dc.create("span",{innerHTML:"车号",style:{"margin-right":"5px"}}),'sqdcdiv');
	var sqdc_vhic= new TextBox({id:"sqdc_vhic",style:{"width":"8em","margin-right":"15px"}}).placeAt('sqdcdiv');
	var queryCondition = {"vhic":sqdc_vhic,"type":"首汽"};
	var sqdcbutton=new Button({
	        label: "查询",
	        onClick: function(){
				this.showWait();
...
//	}).placeAt('sqdcdiv');
	var columns = [
		...
		var xhrArgs = {
			...
			postData : 'postData={"page":1,"pageSize":30,"type":"首汽"}',
			handleAs : "json",
			load : function(data) {
				sqdcbutton.hideWait();
				console.log(data.count)
					sqdcGrid.totalCount = data.count;
					...
					sqdcGrid.set('collection',store);
					sqdcGrid.pagination.refresh(data.datas.length);
			},
		};
		var CustomGrid = declare([Grid, Keyboard ,ColumnResizer,Selection]);
		sqdcGrid= new CustomGrid({
			totalCount : 0,
			columns: columns,
			pagination:null
		},"sqdctable");
		sqdcGrid.refresh2 = function(){
			dojo.xhrPost(xhrArgs);
		}
//			dojo.xhrPost(xhrArgs);
		
		var pageii = new Pagination(sqdcGrid,xhrArgs,queryCondition,30);
		
		dc.place(pageii.first(),'sqdctable','after');
});
