...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
	...
	//加载查询条件按钮
	var dcpjyysyfx_time= new TextBox({id:"dcpjyysyfx_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('dcpjyysyfxdiv');
	var queryCondition = {"time":dcpjyysyfx_time};
	var dcpjyysyfxbutton=new Button({
	        label: "查询",
	        onClick: function(){
				this.showWait();
	        	...
	        	for(var o in queryCondition){
	        		if(o == "time"){
	        			postData[o] = dojo.byId("dcpjyysyfx_time").value;
	        		}else{
	...
	new Button({
		...
		onClick : function() {
			...
			url = "hygl/findpjyysyexcle?postData="
	...
	//查询按钮
	var xhrArgs = {
			url : basePath + "hygl/findpjyysy",
			postData : 'postData={"page":1,"pageSize":'+pageSize+'}',
			handleAs : "json",
			load : function(data) {
				dcpjyysyfxbutton.hideWait();
				var jint = [];
			var zuot = [];
			var qiant=[];
			var shangz=[];
			...
			var min=[];
				...
				dcpjyysyfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
				store = createSyncStore({
					data : data.DATA
				});
				dcpjyysyfxGrid.set('collection', store);
			},
		};
...
function dcpjyysyfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){		
	var myChart = echarts.init(document.getElementById('dcpjyysyfxtb'));
    // 使用
	option = {
		    title : {
		        text: '',
	...
	echarts.init(document.getElementById('dcpjyysyfxtb')).setOption(option);				
}	
function tjnr(list){
	var zhsz=[];
	for(var i=0;i<24;i++){
