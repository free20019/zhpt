...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
	...
	//加载查询条件按钮
	dc.place(dc.create("span",{innerHTML:"日期",style:{"margin-right":"5px"}}),'shizailvdiv');
	var shizailv_time= new TextBox({id:"shizailv_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('shizailvdiv');
	var queryCondition = {"time":shizailv_time};
	var shizailvbutton=new Button({
	        label: "查询",
	        onClick: function(){
				this.showWait();
	        	...
	        	for(var o in queryCondition){
	        		if(o == "time"){
	        			postData[o] = dojo.byId("shizailv_time").value;
	        		}else{
	...
	//查询按钮
	var xhrArgs = {
			url : basePath + "hygl/szl",
			postData : 'postData={"page":1,"pageSize":'+pageSize+'}',
			handleAs : "json",
			load : function(data) {
				shizailvbutton.hideWait();
				var jint = [];
			var zuot = [];
			var qiant=[];
			var shangz=[];
			...
			var min=[];
				...
				console.log(jint)
				console.log(pingj)
				shizailvtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
				store = createSyncStore({
					data : data.DATA
				});
				shizailvGrid.set('collection', store);
			},
		};
...
function shizailvtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){		
	var myChart = echarts.init(document.getElementById('shizailvtb'));
    // 使用
	...
	echarts.init(document.getElementById('shizailvtb')).setOption(option);				
}	
function tjnr(list){
	var zhsz=[];
	for(var i=0;i<48;i++){
		if(list['y'+i]!=null){
			zhsz.push(list['y'+i].substring(0,5));
		}
	}
	return zhsz;
}