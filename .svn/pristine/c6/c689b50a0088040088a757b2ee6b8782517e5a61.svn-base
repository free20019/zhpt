...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
    ...
    //加载查询条件按钮
    var dcpjxszlcfx_time= new TextBox({id:"dcpjxszlcfx_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('dcpjxszlcfxdiv');
    var queryCondition = {"time":dcpjxszlcfx_time};
    var dcpjxszlcfxbutton=new Button({
        label: "查询",
        onClick: function(){
            this.showWait();
            ...
            for(var o in queryCondition){
                if(o == "time"){
                    postData[o] = dojo.byId("dcpjxszlcfx_time").value;
                }else{
    ...
    new Button({
        ...
        onClick : function() {
            ...
            url = "hygl/finddcxsexcle?postData="
    ...
    //查询按钮
    var xhrArgs = {
        url : basePath + "hygl/finddcxs",
        postData : 'postData={"page":1,"pageSize":'+pageSize+'}',
        handleAs : "json",
        load : function(data) {
            dcpjxszlcfxbutton.hideWait();
            console.info(data);
            var jint = [];
            var zuot = [];
            var qiant=[];
            ...
            dcpjxszlcfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
            store = createSyncStore({
                data : data.DATA
            });
            dcpjxszlcfxGrid.set('collection', store);
        },
    };
    ...
    dcpjxszlcfxGrid = new CustomGrid({
        columns : columns
    }, "dcpjxszlcfxtable");
...
function dcpjxszlcfxtb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){
    var myChart = echarts.init(document.getElementById('dcpjxszlcfxtb'));
    // 使用
    option = {
        title : {
            text: '',
    ...
    echarts.init(document.getElementById('dcpjxszlcfxtb')).setOption(option);
}
function tjnr(list){
    var zhsz=[];
    for(var i=0;i<24;i++){
