...
require([ "app/util", "dgrid/Editor","dijit/form/Button" ,"dijit/form/DateTextBox","dijit/form/Select","dijit/form/Textarea","dijit/form/FilteringSelect"
    ...
    //加载查询条件按钮
    var dcpjzklc_time= new TextBox({id:"dcpjzklc_time",value:setformat(new Date()),onClick:function(){WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})},style:{"width":"184px","margin-right":"15px"}}).placeAt('dcpjzklcdiv');
    var queryCondition = {"time":dcpjzklc_time};
    var dcpjzklcbutton=new Button({
        label: "查询",
        onClick: function(){
            this.showWait();
            ...
            for(var o in queryCondition){
                if(o == "time"){
                    postData[o] = dojo.byId("dcpjzklc_time").value;
                }else{
    ...
    new Button({
        ...
        onClick : function() {
            ...
            url = "hygl/finddclcexcle?postData="
    ...
    //查询按钮
    var xhrArgs = {
        url : basePath + "hygl/finddclc",
        postData : 'postData={"page":1,"pageSize":'+pageSize+'}',
        handleAs : "json",
        load : function(data) {
            dcpjzklcbutton.hideWait();
            console.info(data);
            var jint = [];
            var zuot = [];
            var qiant=[];
            ...
            dcpjzklctb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min);
            store = createSyncStore({
                data : data.DATA
            });
            dcpjzklcGrid.set('collection', store);
        },
    };
...
function dcpjzklctb(jint,zuot,qiant,shangz,pingj,shangy,shangm,max,min){
    var myChart = echarts.init(document.getElementById('dcpjzklctb'));
    // 使用
    option = {
        title : {
            text: '',
    ...
    echarts.init(document.getElementById('dcpjzklctb')).setOption(option);
}
function tjnr(list){
    var zhsz=[];
    for(var i=0;i<24;i++){
